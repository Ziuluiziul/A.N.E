...
class FakeAdapter:
    def __init__(
        ...
        *,
        ...
        empty_vote_remaining: list[int] | None = None,
    ) -> None:
        ...
        self.empty_vote_remaining = empty_vote_remaining
    ...
    async def generate(
        ...
        *,
        ...
        if endpoint_id in self.empty_endpoints or self._esvazia_voto(prompt):
    ...
    def _esvazia_voto(self, prompt: str) -> bool:
        if not self.empty_vote_remaining or self.empty_vote_remaining[0] <= 0:
            return False
        try:
            papel = self._role(prompt)
        except AssertionError:
            return False
        if papel in {"proponente", "arbitro"}:
            return False
        self.empty_vote_remaining[0] -= 1
        return True
    ...
    adapters = {
        provider: cast(
            ...
            FakeAdapter(
                ...
                empty_vote_remaining=empty_vote_remaining,
            ),
        )
...
_BANCO_REPLETO = ENDPOINTS + (
    ("nvidia", "zeta-1", "zeta"),
    ("groq", "eta-1", "eta"),
    ("nvidia", "theta-1", "theta"),
    ("groq", "iota-1", "iota"),
    ("nvidia", "kappa-1", "kappa"),
)


async def test_lote_de_votos_vazios_reposta_uma_rodada(tmp_path: Path) -> None:
    """Primeiro lote e uma reposição vazios param: não pede terceira onda abaixo do mínimo."""
    orchestrator, calls, _store = build_orchestrator(
        tmp_path,
        votes={
            "verificador-factual": "approve",
            "critico-epistemologico": "approve",
        ...
        entries=_BANCO_REPLETO,
        empty_vote_remaining=[12],
        max_calls=16,
    )
    ...
    contados = [voto for voto in panel.votes if counted_vote(voto)]
    assert len(contados) < 3
    vazios = [voto for voto in panel.votes if not voto.schema_valid]
    papeis = FakeAdapter._role
    votos_chamados = [
        endpoint for _, endpoint, prompt in calls if papeis(prompt) != "proponente"
    ]
    # 3 iniciais + até 3 na única reposição; sem terceira/quarta onda.
    assert len(votos_chamados) == 6
    assert len(set(votos_chamados)) == 6
    assert len(panel.votes) == 6
    assert len(vazios) == 6


async def test_reposicao_de_voto_para_quando_nao_ha_substituto(tmp_path: Path) -> None:
    """Sem substituto diverso o loop encerra: não insiste no mesmo endpoint."""
    orchestrator, calls, _store = build_orchestrator(
        tmp_path,
        votes={
            "verificador-factual": "approve",
    ...
    assert [voto for voto in panel.votes if counted_vote(voto)] == []
    papeis = FakeAdapter._role
    votos_chamados = [
        endpoint for _, endpoint, prompt in calls if papeis(prompt) != "proponente"
    ]
    assert votos_chamados
    assert len(votos_chamados) == len(set(votos_chamados))
    assert len(calls) == 1 + len(votos_chamados)
    assert len(calls) <= len(_BANCO_REPLETO)
