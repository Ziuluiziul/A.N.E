...
Em decisões com `abstain`, a que decidiu o veredito foi a presença de evidência direta e verificável: sem ela, nenhum claim foi promovido a `established` por maioria ou por analogia. A votação nominal não altera a classificação epistêmica de afirmação alguma.

- `CLM-COMP-CPX-001`, `CLM-COMP-CPX-003` e `CLM-COMP-CPX-004` foram mantidos como `established` porque há provas publicadas e não contestadas (Cook–Levin; teoremas de hierarquia; discussão de robustez em Arora & Barak). A diferença de evidência, portanto, sustentou `approve`.
- `CLM-COMP-CPX-002` permanece `open`. Nenhuma evidência demonstra `P ≠ NP` ou `P = NP` — a ausência de prova não é refutação, e a conjectura não foi verificada. Qualquer voto `approve` nesse claim dependeria de padrão mais alto do que a evidência atual permite; por isso, `abstain` foi a decisão correta, e a classificação `open` permanece.
